...
import {KeypointsOneEuroFilter} from '../filters/keypoints_one_euro_filter';
import {LowPassFilter} from '../filters/low_pass_filter';
...
class MoveNetDetector implements PoseDetector {
  ...
  constructor(
    ...
    this.enableSmoothing = Boolean(config.enableSmoothing);
    ...
    this.enableTracking = Boolean(config.enableTracking);
    if (this.multiPoseModel && this.enableTracking) {
      if (config.trackerType === TrackerType.Keypoint) {
        this.tracker = new KeypointTracker(config.trackerConfig as TrackerConfig);
      } else if (config.trackerType === TrackerType.BoundingBox) {
        this.tracker = new BoundingBoxTracker(config.trackerConfig as TrackerConfig);
      }
  ...
  async estimateMultiplePoses(
    ...
    if (this.enableTracking && this.tracker) {
      this.tracker.apply(poses, timestamp);

      if (this.enableSmoothing) {
        for (let i = 0; i < poses.length; ++i) {
          if (!this.keypointFilterMap.has(poses[i].id as number)) {
            this.keypointFilterMap.set(
                poses[i].id as number,
                new KeypointsOneEuroFilter(KEYPOINT_FILTER_CONFIG));
          }
          poses[i].keypoints =
              // @ts-ignore
              this.keypointFilterMap.get(poses[i].id as number)
  ...
  filterCropRegion(newCropRegion: BoundingBox): BoundingBox | null {
    if (!newCropRegion) {
...
export async function load(modelConfig: MoveNetModelConfig = MOVENET_CONFIG):
  ...
  if (!!config.modelUrl) {
    ...
    if (!modelUrl) {
      throw new Error('modelUrl not selected')
    }
